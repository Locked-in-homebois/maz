...
export default function Carousel() {
	...
	return (
		<div className="bg-sky-900 pt-10 text-logocolor mt-10 mb-1000">
			<MaxWidthWrapper>
				<div className="grid gap-5 max-w-6xl">
					<h1 className="text-5xl bg-sky-00 text-logocolor w-fit rounded-xl px-2 py-1">
						{HEADERCONTENT.header}
					</h1>
					<p className="text-sky-0 text-xl">{HEADERCONTENT.desc}</p>
				</div>
			</MaxWidthWrapper>
					...
					{track.map((img, index) => (
						...
						>
							<Image
								...
								height={300}
								width={400}
								className="rounded-lg overflow-hidden object-cover md:h-90 md:w-100 h-50 w-40 shadow-sky-900 shadow-lg"
							/>
						</motion.div>
					))}
				</motion.div>
