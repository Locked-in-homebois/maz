...
import RedirectCarousel from "./RedirectCarousel";

const Redirect = async () => {
    const t = await getTranslations();
    return (
        <MaxWidthWrapper>
            <div className="grid md:grid-cols-2 pt-10 pb-5 md:gap-0 gap-20 md:px-5 border-2 border-logocolor rounded-2xl my-10">
                    ...
                    <div className="relative grid gap-6 px-10" key={index}>
                        ...
                        <div className="">
                            {item.slides && item.slides.length > 0 ? (
                                <RedirectCarousel slides={item.slides} />
                            ) : (
                                <Image
                                    src={item.image.src}
                                    alt={item.image.alt}
                                    width={1024}
                                    height={1200}
                                    className="object-cover max-h-100 rounded-2xl"
                                />
                            )}
                        </div>
                        <Link href={item.href}>
